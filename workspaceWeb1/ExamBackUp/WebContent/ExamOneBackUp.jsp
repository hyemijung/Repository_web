	...
	function registerFnc() {
	 	...
	 	if (idVal == '') {
	 		...
	 		idVal.focus();
	 	 	...
	 	 	alert('이름을 입력하세요!');
		} else if ( (pwdVal == '') && (nameVal != '')){
			alert('비밀번호를 입력하세요!')
		} else if ( (pwdChkVal == '')  && (pwdVal != '')) {
			alert('비밀번호 확인란을 입력하세요!');
		} else if ( (pwdVal != pwdChkVal)  && (pwdChkVal != '') ){
			alert('패스워드가 틀립니다. 다시 확인하세요!!')
		} else if ( (idVal != '')  && (pwdVal == pwdChkVal) ) {
	...
	function registerFnc() {
	 	...
	 	if (idVal == '') {
	 		...
	 		idVal.focus();
	 	 	...
	 	 	alert('이름을 입력하세요!');
		} else if ( (pwdVal == '') && (idVal != '') && (nameVal != '')){
			alert('비밀번호를 입력하세요!')
		} else if ( (pwdChkVal == '') && (idVal != '') && (nameVal != '') && (pwdVal != '')) {
			alert('비밀번호 확인란을 입력하세요!');
		} else if ( (pwdVal != pwdChkVal) && (idVal != '') && (nameVal != '') && (pwdVal != '') && (pwdChkVal != '') ){
			alert('패스워드가 틀립니다. 다시 확인하세요!!')
		} else if ( (idVal != '') && (nameVal != '') && (pwdVal != '') && (pwdChkVal != '') && (pwdVal == pwdChkVal) ) {
			alert('성공적으로 가입이 되었습니다.');
		}	
		...
		<table id='userAccountTable'>
			...
			<tr>
				...
				<td><input type="password" id='pwdChk' class='pwdChk'
					name="pwdChk" size="15" value=""></td>
			</tr>
			<tr>
				<td colspan="2" style="text-align: right;"><input type="button"
					value="가입" onclick="registerFnc();"> <input type="reset"
					value="취소"></td>
			</tr>
